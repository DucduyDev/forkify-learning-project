...
export const loadSearchRecipes = async function (keyword) {
  try {
    const data = await AJAX(`${API_URL}?search=${keyword}&key=${API_KEY}`);

    if (data.results === 0)
      throw new Error("Could not find any recipes, Please try another one!");
    ...
    const ingredients = Object.entries(newRecipe)
      ...
      .map(ing => {
        ...
        if (ingredient.length !== 3)
          throw new Error("Wrong ingredient format! (unit,quantity,description)");
